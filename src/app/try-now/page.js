    ...
    const calculateResults = () => {
        return questionsData.map((question, index) => ({
            ...
            isCorrect: question.Type === "CheckBox" ?
                JSON.stringify(Object.keys(userAnswers[index] || {}).sort()) === JSON.stringify(question.CorrectAnswer.sort()) :
                ...
                <div className="max-w-md p-8 bg-white rounded-lg shadow-lg">
                    <div className="flex flex-col">
                        <div className="mb-2 text-center">
                            <h3 className="text-2xl font-bold">{Math.ceil(timeLeft / 1000)} seconds left</h3>
                        </div>
                        <div className="bg-gray-200 rounded-full w-full h-2.5 mt-2">
                            <div className="bg-blue-500 h-full rounded-full transition-all duration-300" style={{ width: `${timeLeftPercent}%` }}></div>
                        </div>
                    </div>
                            ...
                            {currentQuestion.Options.map((option, index) => (
                                <div key={index} className="flex items-center mb-2">
                                    <input
                                        ...
                                        className="accent-blue-500"
                                    />
                                    <label htmlFor={`image-option-${index}`} className="ml-2 text-gray-700">{option}</label>
                                </div>
                            ))}
                        </div>
                    )}
